...
def train_detr(train_loader, epochs=5, learning_rate=0.001):
    model_name = "DETR"
    ...
    detr.class_labels_classifier = nn.Linear(detr.class_labels_classifier.in_features, 10)  # Replace classification head
    detr = detr.to(device)

    optimizer = optim.Adam(detr.parameters(), lr=learning_rate)
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (images, labels) in enumerate(train_loader):
            ...
            optimizer.zero_grad()
            outputs = detr(images)
            logits = outputs.logits[:, 0, :]  # Use only the first object's logits for classification
        ...
        # Save model checkpoint after each epoch with specific naming
        os.makedirs("saved_models", exist_ok=True)
        torch.save(detr.state_dict(), f"saved_models/{model_name}_epoch_{epoch+1}.pth")  # Save with epoch info
    ...
    # Log simple summary
    parameter_count = sum(p.numel() for p in detr.parameters())
    ...
    torch.save(detr.state_dict(), f"saved_models/{model_name}_final.pth")
    log_message(f"{model_name} final model saved at 'saved_models/{model_name}_final.pth'", model_name, epochs, learning_rate)
